...
export async function generateMetadata({ params }: Props) {
  ...
  return { title: user?.displayName ? `Following · ${user.displayName}` : `Following · @${username}` };
}

export default async function PublicFollowingPage({ params }: Props) {
  const { username } = await params;
  ...
  return (
    <div style={{ maxWidth: 560, margin: "0 auto", padding: "40px 24px 80px" }}>
      ...
      <h1 style={{ fontSize: 22, fontWeight: 800, color: "var(--text)", margin: "16px 0 8px" }}>Following</h1>
      <p style={{ fontSize: 14, color: "var(--muted)", marginBottom: 24 }}>
        People {user.displayName ?? `@${user.username}`} follows.
      </p>

      {members.length === 0 ? (
        <p style={{ fontSize: 14, color: "var(--muted)" }}>Not following anyone yet.</p>
      ) : (
