...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const comments = await prisma.feedPostComment.findMany({
    where: { feedPostId: id, deletedAt: null },
  ...
  const attMap = await loadAttachmentsGrouped(
    prisma,
    PostAttachmentParent.feed_post_comment,
    comments.map((c) => c.id),
  );
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const comment = await prisma.$transaction(async (tx) => {
    const c = await tx.feedPostComment.create({
      data: { feedPostId: id, userId: session.user.id, body },
      include: { user: { select: { id: true, username: true, displayName: true } } },
    });
    await replacePostAttachments(tx, PostAttachmentParent.feed_post_comment, c.id, attachmentRows);
  ...
  const attachments = await prisma.postAttachment.findMany({
    where: { parentType: PostAttachmentParent.feed_post_comment, parentId: comment.id },
