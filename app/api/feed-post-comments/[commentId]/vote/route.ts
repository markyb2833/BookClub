...
import { setFeedPostCommentVote } from "@/lib/social/binaryVote";
import { z } from "zod";
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ commentId: string }> }) {
  ...
  const comment = await prisma.feedPostComment.findFirst({
  ...
  await setFeedPostCommentVote(prisma, commentId, session.user.id, parsed.data.value);

  const next = await prisma.feedPostComment.findUnique({
    where: { id: commentId },
    select: {
      upvotesCount: true,
      downvotesCount: true,
