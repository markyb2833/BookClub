...
import { setReviewCommentVote } from "@/lib/social/binaryVote";
import { z } from "zod";
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ commentId: string }> }) {
  ...
  await setReviewCommentVote(prisma, commentId, session.user.id, parsed.data.value);

  const next = await prisma.reviewComment.findUnique({
    where: { id: commentId },
    select: {
      upvotesCount: true,
      downvotesCount: true,
