...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ reviewId: string }> }) {
  ...
  const attMap = await loadAttachmentsGrouped(
    prisma,
    PostAttachmentParent.review_comment,
    comments.map((c) => c.id),
  );
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ reviewId: string }> }) {
  ...
  const comment = await prisma.$transaction(async (tx) => {
    const c = await tx.reviewComment.create({
      data: { reviewId, userId: session.user.id, body },
      include: { user: { select: { id: true, username: true, displayName: true } } },
    });
    await replacePostAttachments(tx, PostAttachmentParent.review_comment, c.id, attachmentRows);
  ...
  const attachments = await prisma.postAttachment.findMany({
    where: { parentType: PostAttachmentParent.review_comment, parentId: comment.id },
