...
import { setReviewVote } from "@/lib/social/binaryVote";
import { z } from "zod";
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ reviewId: string }> }) {
  ...
  await setReviewVote(prisma, reviewId, session.user.id, parsed.data.value);

  const next = await prisma.review.findUnique({
    where: { id: reviewId },
    select: {
      upvotesCount: true,
      downvotesCount: true,
