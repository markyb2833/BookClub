...
import { setBookRecommendationVote } from "@/lib/social/binaryVote";
import { z } from "zod";
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const row = await prisma.bookRecommendation.findFirst({
  ...
  await setBookRecommendationVote(prisma, id, session.user.id, parsed.data.value);

  const next = await prisma.bookRecommendation.findUnique({
    where: { id },
    select: {
      upvotesCount: true,
      downvotesCount: true,
